...
class DQAgent:
    ...
    def __init__(self, args):
        ...
        if not self.training:
            cwd = os.getcwd()
            self.model.load_state_dict(torch.load(os.path.join(cwd, args.model_file), map_location=self.device), strict=False)
            self.model.eval()
    ...
    def select_action(self, graph, state, epsilon, training=True, budget=None):
        ''' act upon state '''
        # --- REMOVED: Action Masking / Quota Enforcement Logic ---
