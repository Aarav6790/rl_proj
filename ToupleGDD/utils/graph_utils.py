...
def read_graph(path, ind=0, directed=False):
    ''' method to load edge as node pair graph (Memory Efficient Version) '''
    print("  Pass 1: Identifying unique nodes...")
    nodes = set()
    with open(path, 'r') as f:
        for line in f:
            if line.startswith(('#', '%')) or not line.strip():
                continue
            row = line.split()
            nodes.add(int(row[0]) - ind)
            nodes.add(int(row[1]) - ind)

    # Relabel nodes to be contiguous
    num_nodes = len(nodes)
    mapping = {node: i for i, node in enumerate(nodes)}
    nodes = None # Free up memory
    
    parents = {i: set() for i in range(num_nodes)}
    children = {i: set() for i in range(num_nodes)}
    edges = {}

    print(f"  Pass 2: Building graph structure ({num_nodes} nodes)...")
    with open(path, 'r') as f:
        for line in f:
            if line.startswith(('#', '%')) or not line.strip():
                continue
            row = line.split()
            s = mapping[int(row[0]) - ind]
            d = mapping[int(row[1]) - ind]
            ...
            if not directed:
                # Add edge d -> s
                if s not in children[d]:
                    children[d].add(s)
                    parents[s].add(d)
                    edges[(d, s)] = 0.0

    print("  Calculating edge probabilities...")
    # change the probability to 1/indegree
    for (s, d) in edges:
        edges[(s, d)] = 1.0 / len(parents[d])
            
    return Graph(set(range(num_nodes)), edges, children, parents)
